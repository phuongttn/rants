# This file is adapted from the original HaMeR implementation.
# RANTS reuses the overall HaMeR training, optimization, and MANO-based
# regression pipeline, while allowing modifications in the backbone for
# token sparsification.
#
# Main reference:
#   G. Pavlakos et al., "Reconstructing Hands in 3D with Transformers,"
#   CVPR 2024.
...
class RANTS(pl.LightningModule):

    def __init__(self, cfg: CfgNode, init_renderer: bool = True):
        """
        Setup the RANTS model.

        This implementation is adapted from the original HaMeR framework.
        RANTS retains the overall HaMeR pipeline for backbone feature extraction,
        MANO parameter regression, rendering, and optimization, while allowing
        modifications in the backbone for token sparsification.

        Args:
            cfg (CfgNode): Configuration file as a yacs CfgNode.
            init_renderer (bool): Whether to initialize the renderers used for
                visualization during training and validation.
        """
        super().__init__()
        ...
        # In RANTS, the main architectural modifications are typically introduced here
        # by replacing or extending the original HaMeR backbone with a token-sparsified variant.
        ...
        # Create the MANO regression head that predicts hand parameters from backbone features.
        ...
        # Define the supervised losses used for optimization.
        self.keypoint_3d_loss = Keypoint3DLoss(loss_type='l1')
        self.keypoint_2d_loss = Keypoint2DLoss(loss_type='l1')
        self.mano_parameter_loss = ParameterLoss()

        # Instantiate the MANO hand model used to recover 3D joints and mesh vertices.
    ...
    def forward_step(self, batch: Dict, train: bool = False) -> Dict:
        ...
        The global regression pipeline follows HaMeR. Any RANTS-specific
        sparsification is expected to happen inside the backbone.
        Args:
        ...
        # Use the cropped RGB image as input to the model.
        x = batch['img']
        batch_size = x.shape[0]

        # Extract conditioning features using the backbone.
        # As in HaMeR, the ViT backbone operates on a center-cropped input
        # with a different aspect ratio.
        conditioning_feats = self.backbone(x[:,:,:,32:-32])

        # Predict MANO parameters and weak-perspective camera parameters.
        ...
        # Convert the weak-perspective camera prediction into a 3D camera translation.
        ...
        # Recover MANO vertices and joints, then project the 3D joints to the image plane.
    ...
    def compute_loss(self, batch: Dict, output: Dict, train: bool = True) -> torch.Tensor:
        ...
        # Read the supervision signals from the batch.
